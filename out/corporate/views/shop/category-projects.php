...
<!-- Category -->
<?php $obj= new ebapps\corporate\corporate(); $obj -> project_thurmnail_category_category($projectCategory); ?>
<?php if($obj->data){ foreach($obj->data as $val): extract($val); ?> 
<button class='eb-filter-back filter-button' data-filter='<?php echo $project_sub_category; ?>'><?php echo ucfirst($obj->visulString($project_sub_category)); ?></button>
<?php endforeach; ?>
<?php } ?>
</div>
</div>
<div class='row'>
<?php $obj= new ebapps\corporate\corporate(); $obj -> project_thurmnail_category_details($projectCategory); ?>
<?php if($obj->data){ foreach($obj->data as $val): extract($val); ?> 
...
<div class='cap1'>
<h3 title='<?php echo ucfirst($obj->visulString($project_category)); ?>'><?php echo ucfirst($obj->visulString($project_category)); ?></h3>
